(function(angular) {
	var officeEntry;
	var officeEntryReportController = function($scope, moment, employeeAPIservice, ebsReportAPIservice){
		$scope.data = {};
	    $scope.stDate = {
	        opened: false
	    };
    	...
    	$scope.searchLeave = function(){
    		ebsReportAPIservice.loadEbsLeaveReport($scope.data).success(function(response) {
    			officeEntry.dtInstance.DataTable.clear().draw();
    			var dataArray;
    			$.each(response.entity, function(i, val){
    				response.entity[i].class
    			});
    			officeEntry.dtInstance.DataTable.rows.add(response.entity).draw();
	        });
	    };
	...
	officeEntryReportController.$inject = ['$scope', 'moment', 'employeeAPIservice', 'ebsReportAPIservice'];
	mainApp.controller('officeEntryReportController', officeEntryReportController);
	
	function officeEntryReportControllerTable($scope, $compile, DTOptionsBuilder, DTColumnBuilder, ebsReportAPIservice) {
		officeEntry = this;
		officeEntry.title = 'OfficeEntry Reports';
		officeEntry.dtColumns = [
	...
	officeEntryReportControllerTable.$inject = ['$scope', '$compile', 'DTOptionsBuilder','DTColumnBuilder', 'ebsReportAPIservice'];
	mainApp.controller('officeEntryReportControllerTable', officeEntryReportControllerTable);
})(angular);